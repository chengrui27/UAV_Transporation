...
def build_initial_state(params, n_cables: int):
    ...
    # x0[13 : 16] = np.array([-1.0, 0.0, 0.0])  
    # x0[27 : 30] = np.array([0.0, -1.0, 0.0])  
    # x0[41 : 44] = np.array([0.0, 1.0, 0.0])  
...
def create_nmpc_solver(n_cables: int = 3):
    ...
    # 无人机推力约束（使用 model.con_h_expr, 仅包含 T_i^2）
    # 每架无人机质量 2kg，最大推力近似 32N => T_max^2
    T_max = 32.0
    n_thrust = n_cables
    lh = np.zeros(n_thrust)
    uh = np.zeros(n_thrust)
