...
System: 3 quadrotors transporting a triangular prism payload via cables
"""

import numpy as np
from acados_template import AcadosModel
from casadi import SX, vertcat, cos, sin, norm_2, cross, mtimes, sqrt, inv
...
def create_multi_payload_model():
    ...
    - Payload: Triangular prism (height 0.4m, side length 3m, mass 1kg)
    - 3 quadrotors with cables attached at the three corners at mid-height
    ...
    - Quadrotors: quaternion qRi (4) for each quadrotor, i=1,2,3

    Total states: 13 (load) + 18 (3 cables) + 12 (3 quadrotors) = 43

    Control Inputs (Modified from paper):
    - Thrust fi (1) and angular velocity Omega_i (3) for each quadrotor, i=1,2,3
    - Note: Angular velocities are direct control inputs (not computed from moments)
    Total controls: 12
    """
    ...
    # Load parameters (triangular prism)
    m_L = 1.0  # Mass of load [kg]
    h_prism = 0.4  # Height of triangular prism [m]
    a_prism = 3.0  # Side length of triangular base [m]

    # Inertia matrix of triangular prism (body frame)
    # For a triangular prism with equilateral triangle base
    # Ix = Iy = m(h^2/12 + a^2/18), Iz = m*a^2/9
    J_Lxx = m_L * (h_prism**2 / 12 + a_prism**2 / 18)
    J_Lyy = J_Lxx
    J_Lzz = m_L * a_prism**2 / 9
    J_L = np.diag([J_Lxx, J_Lyy, J_Lzz])

    # Quadrotor parameters
    m_i = 1.535  # Mass of each quadrotor [kg]
    J_i = np.diag([0.0023, 0.0023, 0.004])  # Inertia matrix of quadrotor

    # Cable parameters
    # Cables attached at the three corners of the triangular base at mid-height
    # Positions in body frame of load
    radius = a_prism / np.sqrt(3)  # Radius of circumscribed circle
    r_b = np.zeros((3, 3))
    for i in range(3):
        angle = i * 2 * np.pi / 3  # 0, 120, 240 degrees
        r_b[i, 0] = radius * np.cos(angle)
        r_b[i, 1] = radius * np.sin(angle)
        r_b[i, 2] = 0.0  # At mid-height (body frame origin at CoM)

    # Cable lengths
    L_cable = np.array([1.5, 1.5, 1.5])  # Cable length [m]
    ...
    omega_cables = []  # Angular velocities �i
    ...
    # Quadrotor states (attitudes as quaternions only, angular velocity is now control input)
    ...
    # Control Inputs
    # ------------------------
    # Control: thrust and angular velocity for each quadrotor
    controls = []
    f_thrust = []
    Omega_R = []  # Angular velocities are now control inputs
    for i in range(n_quad):
        f_thrust.append(SX.sym(f'f_{i+1}'))  # Thrust magnitude
        Omega_R.append(SX.sym(f'Omega_R{i+1}', 3))  # Angular velocity (control input)
        controls.append(f_thrust[i])
        controls.append(Omega_R[i])
    ...
    for i in range(n_quad):
        ...
        # Omega_R[i] is now a control input, not a state
