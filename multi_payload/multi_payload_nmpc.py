...
System: 3 quadrotors carrying a triangular prism payload via cables
"""
...
def create_nmpc_solver():
    ...
    # Cable direction and angular velocity weights (13-30: 3 cables x 6 dims)
    for i in range(13, 31):
        Q[i, i] = 5.0

    # Quad attitude quaternion weights (31-42: 3 quads x 4 dims)
    # Note: Quad angular velocities are now control inputs, not states
    for i in range(31, 43):
        Q[i, i] = 20.0

    # Control input weight matrix (12 dims: 3 quads x (1 thrust + 3 angular velocities))
    R = np.eye(nu)
    for i in range(3):  # 3 quadrotors
        R[4*i, 4*i] = 5.0       # thrust
        R[4*i+1, 4*i+1] = 20.0  # Omega_x (angular velocity)
        R[4*i+2, 4*i+2] = 20.0  # Omega_y (angular velocity)
        R[4*i+3, 4*i+3] = 20.0  # Omega_z (angular velocity)
    ...
    # Quadrotor reference attitude (unit quaternion only, angular velocity is now control input)
    for i in range(3):
        yref[31 + 4*i : 31 + 4*i + 4] = [1.0, 0.0, 0.0, 0.0]  # qRi (4 dims per quad)

    # Reference control input (hover thrust)
    g = params['g']
    m_L = params['m_L']
    m_i = params['m_i']
    total_mass = m_L + 3 * m_i
    thrust_hover = (total_mass * g) / 3.0  # 3 quadrotors share weight

    for i in range(3):
        yref[nx + 4*i] = thrust_hover  # thrust
        yref[nx + 4*i + 1 : nx + 4*i + 4] = [0.0, 0.0, 0.0]  # angular velocity
    ...
    ubu = []
    for i in range(3):  # 3 quadrotors
        lbu.extend([0.0, -3.0, -3.0, -3.0])  # thrust lower, angular velocity lower
        ubu.extend([30.0, 3.0, 3.0, 3.0])    # thrust upper, angular velocity upper
    ...
    # Quadrotor initial attitude (unit quaternion: level, angular velocity is now control input)
    for i in range(3):
        x0[31 + 4*i : 31 + 4*i + 4] = [1.0, 0.0, 0.0, 0.0]  # qRi (4 dims per quad)
...
def nmpc_sim():
    ...
    # Quadrotor initial attitude (level, angular velocity is now control input)
    for i in range(3):
        x0[31 + 4*i : 31 + 4*i + 4] = [1.0, 0.0, 0.0, 0.0]
...
def visualize_multi_payload(simX, simU, params, interval=50, save_animation=False):
    ...
    r_b = params['r_b']
    h_prism = params['h_prism']
    a_prism = params['a_prism']
    ...
    def update(frame):
        ...
        # Update load (draw triangular prism)
        if load_body is not None:
            load_body.remove()

        # Triangular prism vertices (in load body frame)
        # Bottom triangle
        radius = a_prism / np.sqrt(3)
        bottom_vertices = []
        top_vertices = []
        for i in range(3):
            angle = i * 2 * np.pi / 3
            ...
            bottom_vertices.append([x_local, y_local, -h_prism/2])
            top_vertices.append([x_local, y_local, h_prism/2])

        # Transform to world frame
        bottom_world = [load_pos[frame] + R_L @ np.array(v) for v in bottom_vertices]
        top_world = [load_pos[frame] + R_L @ np.array(v) for v in top_vertices]

        # Build triangular prism faces
        faces = []
        # Bottom face
        faces.append([bottom_world[0], bottom_world[1], bottom_world[2]])
        # Top face
        faces.append([top_world[0], top_world[1], top_world[2]])
        # Side faces
        faces.append([bottom_world[0], bottom_world[1], top_world[1], top_world[0]])
        faces.append([bottom_world[1], bottom_world[2], top_world[2], top_world[1]])
        faces.append([bottom_world[2], bottom_world[0], top_world[0], top_world[2]])
        ...
        # Approximate cable tension (display only)
        if frame < len(simU):
            state_str += f'\nThrust: [{simU[frame, 0]:.1f}, {simU[frame, 4]:.1f}, {simU[frame, 8]:.1f}] N'
