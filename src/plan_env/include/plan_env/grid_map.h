...
struct MappingParameters
{
  ...
  double visualization_truncate_height_,visualization_truncate_low_, virtual_ceil_height_,virtual_ceil_low_, ground_height_, virtual_ceil_yp_, virtual_ceil_yn_;
  bool show_occ_time_;
...
struct MappingData
{
  ...
  // gradient buffer: stores [grad_x, grad_y, grad_z] for each voxel
  std::vector<Eigen::Vector3d> gradient_buffer_;
  ...
  // shared memory for esdf map (distance + gradient)
  // Layout: [dist, grad_x, grad_y, grad_z] for each voxel
  int shm_fd_;
  float* shm_ptr_;  // 4 floats per voxel: [distance, grad_x, grad_y, grad_z]
  size_t shm_size_;
...
public:
  ...
  pair<double, Eigen::Vector3d> evaluateEDTWithGrad(const Eigen::Vector3d& pos,
...
private:
  ...
  // ROS2 message filter sync policies
  typedef message_filters::sync_policies::ApproximateTime<
    sensor_msgs::msg::Image,
    nav_msgs::msg::Odometry> SyncPolicyImageOdom;
  typedef message_filters::sync_policies::ApproximateTime<
    sensor_msgs::msg::Image,
  ...
  rclcpp::Publisher<plan_env::msg::ESDFMap>::SharedPtr esdf_metadata_pub_;
