    ...
    resolution_inv_(10.0)
{
  ...
  RCLCPP_INFO(node_->get_logger(), "ESDF Map Reader initialized, waiting for map metadata...");
}
  ...
  if (shm_name_ != msg->shm_name) {
    ...
    if (openSharedMemory(msg->shm_name)) {
      ...
      RCLCPP_INFO(node_->get_logger(),
        "ESDF Map updated: origin=[%.1f, %.1f, %.1f], resolution=%.2f, voxels=[%d, %d, %d], local_bound=[%d:%d, %d:%d, %d:%d]",
...
bool ESDFMapReader::openSharedMemory(const std::string& shm_name) {
  ...
  shm_ptr_ = (float*)mmap(NULL, shm_size_, PROT_READ, MAP_SHARED, shm_fd_, 0);
  ...
  RCLCPP_INFO(node_->get_logger(), "Shared memory opened: %s, size: %.2f MB",
