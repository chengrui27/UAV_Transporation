...
private:
     ...
     * @param yref   输出参考向量 (NY0 维: 状态 + 控制)
     * @param yref_e 输出终端参考向量 (NYN 维: 仅状态)
     */
    void generatePayloadReference(double* yref, double* yref_e) {
        // 先清零
        for (int i = 0; i < PX4_PAYLOAD_MODEL_NY0; ++i) {
            yref[i] = 0.0;
        }
        for (int i = 0; i < PX4_PAYLOAD_MODEL_NYN; ++i) {
            yref_e[i] = 0.0;
        }
        ...
        // 参考机体系角速度和角速度导数均为 0
        // yref[17], yref[18], yref[19] 对应 Ωx, Ωy, Ωz
        // yref[20], yref[21], yref[22] 对应 Ωdot_x, Ωdot_y, Ωdot_z

        // 参考输入 u = [T_dot, Ωddot_x, Ωddot_y, Ωddot_z] 设为 0
        // 对应 yref[23..26]，前面已经清零
    ...
    bool solveNMPC() {
        ...
        // 准备状态向量 (与 px4_payload_model.py 一致, 23维)
        double x0[PX4_PAYLOAD_MODEL_NX];

        if (first_solve_done_) {
            // 做法一：有上一轮解时，先取上一轮预测的第1个节点作为初始猜测
            // x_prev_1 = x(k+1|k)
            ocp_nlp_out_get(nlp_config_, nlp_dims_, nlp_out_, 1, "x", x0);
        } else {
            // 首次求解：初始化为当前测量 + 合理的内部执行器状态
            for (int i = 0; i < PX4_PAYLOAD_MODEL_NX; ++i) {
                x0[i] = 0.0;
            }
            double hover_thrust = (quad_mass_ + payload_mass_) * gravity_;
            x0[16] = hover_thrust;  // T 初始为悬停推力
            // Ω 和 Ωdot 初始为 0，已在上面清零
        }

        // 使用最新测量覆盖可观测状态 (负载、绳子、姿态，均在 ENU 坐标系)
        ...
        // 生成参考轨迹 (基于RViz目标点)
        double yref[PX4_PAYLOAD_MODEL_NY0];   // 状态 + 控制
        double yref_e[PX4_PAYLOAD_MODEL_NYN]; // 终端状态
        ...
        if (!first_solve_done_) {
            ...
            for (int i = 0; i < N; i++) {
                // 1. 提取该阶段的猜测状态
                double xi[PX4_PAYLOAD_MODEL_NX];  // [xp, yp, zp, vxp, vyp, vzp, qx, qy, qz, wx, wy, wz, q0, q1, q2, q3, ...]
        ...
        for (int iter = 0; iter < max_inner_iters; ++iter) {
            ...
            for (int k = 0; k <= N; ++k) {
                double x_pred[PX4_PAYLOAD_MODEL_NX];  // [xp, yp, zp, ...]
            ...
            if (iter < max_inner_iters - 1) {
                for (int i = 0; i < N; ++i) {
                    double xi[PX4_PAYLOAD_MODEL_NX];
                    ocp_nlp_out_get(nlp_config_, nlp_dims_, nlp_out_, i, "x", xi);
    ...
    void applyStageCostDiscount() {
        ...
        // 折扣因子 gamma \in (0, 1]，仅缩放已有的 W，而不改变其结构
        const double gamma = 0.9;

        // 缓冲区：用于读写每个阶段当前的 W
        double W_stage[PX4_PAYLOAD_MODEL_NY * PX4_PAYLOAD_MODEL_NY];

        // 为每个阶段设置折扣后的 W_k = gamma^k * W_k_original
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Stage cost discount applied: gamma=%.3f (scaled existing W per stage)",
                    gamma);
    }
